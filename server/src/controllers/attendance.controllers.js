...
import { logStudentAttendance } from "../repository/attendance.repository.js";
...
export const logAttendance = async (req, res) => {
    ...
    if (!batchCode || !absentStudents || !subjectCode) {
        return res.status(400).json({ message: "All fields are required [batchCode, absentStudents, subjectCode, remarks]" });
    }
    //@TODO: Validate the batch_code and subject_code
    
    try {
        // get all the students (roll_numbers) from batch_code
        const presentStudents = await getStudentPresentRollNumbersByBatchCode(batchCode, absentStudents);
